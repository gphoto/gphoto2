...
- `http://<server ip>:8866/api/file/get/<path to file/image>` 
  ...
  response - native file, with content-type header is set to recognized image mimetype e.g. `image/jpeg`
  
- `http://<server ip>:8866/api/file/download/<path to file/image>` 

  response - native file, content-type header is allways `application/octet-stream` and the content-disposition filename header is set.
